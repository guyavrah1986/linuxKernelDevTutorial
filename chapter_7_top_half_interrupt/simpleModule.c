...
/*
 *  simpleModule.c: Creates a read-only char device that says how many times
...
#include <linux/interrupt.h>	// for interrupt definition


#define SUCCESS 0
#define DEVICE_NAME "my_simple_dev"				// Device name as it appears in /proc/devices
#define DRIVER_CLASS_NAME "my_simple_char_dev_class"		// class name (for the device) as it appears in /sys/class
...
irqreturn_t simpleCharDevHandler(int irqNum, void* dev)
{
	printk(KERN_INFO "mySimpleCharDev::simpleCharDevHandler - got irqNum:%d and dev points to address:%p \n", irqNum, dev);
	return 0;
}
...
static int __init simpleCharDevInit(void)
{
	...
	printk(KERN_ALERT "mySimpleCharDev::simpleCharDevInit - device got major number:%d\n", major);
	printk(KERN_ALERT "mySimpleCharDev::simpleCharDevInit - Remove the device file and module when done.\n");


	irq_handler_t handler* = simpleCharDevHandler;
	int registerDeviceInterruptHandlerRes = request_irq(/*unsigned int irq*/ 0,
                					    /*irq_handler_t handler*/ handler,
                					    /*unsigned long flags*/ IRQF_SHARED,
                					    /*const char *name*/ "simpleModuleDev",
                					    /*void *dev*/, &dev);

	if (registerDeviceInterruptHandlerRes == 0)
	{
		printk(KERN_ALERT "mySimpleCharDev::simpleCharDevInit - device interrupt handler registered successfully\n");
	}
	else
	{
		printk(KERN_ALERT "mySimpleCharDev::simpleCharDevInit - device interrupt handler did NOT registered successfully\n");
	}
...
static void __exit simpleCharDevExit(void)
{
	// add the removal of the interrupt handler of this device
	free_irq(/*unsigned int irq*/ 0, /*void *dev*/ &dev);
			   ...
			   loff_t* offset)
{
	...
	bytesRead = raw_copy_to_user((void*)buffer, (const void*)bufferToRead, sizeOfBufferToRead);
